...
template <INT_TYPE_FOR_MONTGOMERY M> inline constexpr Montgomery<M>::Montgomery() noexcept : m_n() { static_assert( !Constants::g_even ); }
...
template <INT_TYPE_FOR_MONTGOMERY M> template <typename T> inline constexpr Montgomery<M>::Montgomery( T n ) noexcept : m_n( Form( Residue<M>( move( n ) ) ) ) { static_assert( !Constants::g_even && is_constructible_v<INT_TYPE_FOR_MONTGOMERY,decay_t<T> > ); }
...
template <INT_TYPE_FOR_MONTGOMERY M> template <typename INT> inline constexpr Montgomery<M>& Montgomery<M>::operator<<=( INT n ) { assert( n >= 0 ); return *this *= ( m_n < Constants::g_memory_length ? TwoPower( n ) : Montgomery<M>( 2 ).NonNegativePower( move( n ) ) ); }
...
template <INT_TYPE_FOR_MONTGOMERY M> inline constexpr Montgomery<M>& Montgomery<M>::operator++() noexcept { return Ref( Normalise( m_n += Constants::g_Montgomery_base_mod ) ); }
...
template <INT_TYPE_FOR_MONTGOMERY M> inline constexpr Montgomery<M>& Montgomery<M>::operator--() noexcept { return Ref( ( m_n < Constants::g_Montgomery_base_mod ? m_n += M : m_n ) -= Constants::g_Montgomery_base_mod ); }
template <INT_TYPE_FOR_MONTGOMERY M> inline constexpr Montgomery<M> Montgomery<M>::operator--( int ) noexcept { Montgomery<M> n{ *this }; operator--(); return n; }
...
template <INT_TYPE_FOR_MONTGOMERY M> inline constexpr Montgomery<M>& Montgomery<M>::Invert() { assert( m_n != 0 ); return PositivePower( INT_TYPE_FOR_MONTGOMERY( Constants::g_M_minus_2 ) ); }

template <INT_TYPE_FOR_MONTGOMERY M> template <typename INT> inline constexpr Montgomery<M>& Montgomery<M>::PositivePower( INT exponent ) noexcept { Montgomery<M> power{ *this }; ( --exponent ) %= Constants::g_M_minus_2; while( exponent != 0 ){ ( exponent & 1 ) == 1 ? *this *= power : *this; exponent >>= 1; power *= power; } return *this; }
template <INT_TYPE_FOR_MONTGOMERY M> template <typename INT> inline constexpr Montgomery<M>& Montgomery<M>::NonNegativePower( INT exponent ) noexcept { return exponent == 0 ? Ref( m_n = Constants::g_Montgomery_base_mod ) : PositivePower( move( exponent ) ); }
template <INT_TYPE_FOR_MONTGOMERY M> template <typename INT> inline constexpr Montgomery<M>& Montgomery<M>::Power( INT exponent ) { bool neg = exponent < 0; assert( !( neg && m_n == 0 ) ); return neg ? PositivePower( move( exponent *= Constants::g_M_minus_2_neg ) ) : NonNegativePower( move( exponent ) ); }
...
template <INT_TYPE_FOR_MONTGOMERY M> inline constexpr INT_TYPE_FOR_MONTGOMERY Montgomery<M>::GetModulo() noexcept { return M; }

template <INT_TYPE_FOR_MONTGOMERY M> inline constexpr INT_TYPE_FOR_MONTGOMERY Montgomery<M>::Form( const INT_TYPE_FOR_MONTGOMERY& n ) noexcept { ull n_copy = n; return INT_TYPE_FOR_MONTGOMERY( move( Reduction( n_copy *= Constants::g_Montgomery_base_square_mod ) ) ); }
template <INT_TYPE_FOR_MONTGOMERY M> inline constexpr ull& Montgomery<M>::Reduction( ull& n ) noexcept { ull n_sub = n & Constants::g_Montgomery_base_minus; return ( ( n += ( ( n_sub *= Constants::g_Montgomery_M_neg_inverse ) &= Constants::g_Montgomery_base_minus ) *= M ) >>= Constants::g_Montgomery_digit ) < M ? n : n -= M; }
...
template <INT_TYPE_FOR_MONTGOMERY M> inline constexpr uint Montgomery<M>::BaseSquareTruncation( INT_TYPE_FOR_MONTGOMERY& n ) noexcept { const uint n_u = n >> Constants::g_Montgomery_digit_half; n &= Constants::g_Montgomery_base_sqrt_minus; return n_u; }
