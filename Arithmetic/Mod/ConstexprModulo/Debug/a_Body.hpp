...
template <INT_TYPE_FOR_MOD M> inline const Mod<M>& Mod<M>::Inverse( const INT_TYPE_FOR_MOD& n ) { assert( n < M ); static vector<Mod<M>> memory = { zero() , one() }; static INT_TYPE_FOR_MOD length_curr = 2; while( length_curr <= n ){ memory.push_back( one() ); memory.back().m_d = length_curr++; } return memory[n]; }
