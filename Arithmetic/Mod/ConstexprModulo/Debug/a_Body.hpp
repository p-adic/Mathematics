...
template <INT_TYPE_FOR_MOD M> inline constexpr bool Mod<M>::operator==( const Mod<M>& n ) const noexcept { const uint n0 = ull( m_n ) * n.m_d % M , n1 = n.m_n * ull( m_d ) % M; return ( m_non_negative || n0 == 0 ? n0 : M - n0 ) == ( n.m_non_negative || n1 == 0 ? n1 : M - n1 ); }
...
template <INT_TYPE_FOR_MOD M> inline const Mod<M>& Mod<M>::Inverse( const int& n ) { assert( 0 < n && n < int( Constants::g_memory_length ) ); static vector<Mod<M>> memory = { zero() , one() }; static int length_curr = 2; while( length_curr <= n ){ memory.push_back( one() ); memory.back().m_d = length_curr++; } return memory[n]; }
...
template <INT_TYPE_FOR_MOD M> inline const Mod<M>& Mod<M>::TwoPowerInverse( const int& n ) { assert( 0 <= n && n < int( Constants::g_memory_length ) ); static vector<Mod<M>> memory = { one() }; static int length_curr = 1; while( length_curr <= n ){ memory.push_back( Derepresent( 1 ) / TwoPower( n ) ); length_curr++; } return memory[n]; }
...
template <INT_TYPE_FOR_MOD M> inline constexpr uint Mod<M>::Represent() const noexcept { Mod<M> d_inv = Mod<M>::Derepresent( m_d ).NonNegativePower( Constants::g_orderm ); return ull( m_non_negative ? m_n : M - m_n ) * d_inv.m_n % M; }
