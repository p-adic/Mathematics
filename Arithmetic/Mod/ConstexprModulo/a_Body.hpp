...
template <INT_TYPE_FOR_MOD M> inline constexpr Mod<M>& Mod<M>::operator^=( ll exponent ) { if( exponent < 0 ){ m_n = ModularInverse( M , move( m_n ) ); exponent *= -1; } return NonNegativePower( move( exponent ) ); }
