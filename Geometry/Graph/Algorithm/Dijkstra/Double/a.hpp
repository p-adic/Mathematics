...
// を満たす場合にのみサポート。
template <typename ACYCLIC_GRAPH , typename GRAPH , typename VERTEX , typename T1 = inner_t<ACYCLIC_GRAPH> , typename T2 = decldecay_t( declval<inner_t<GRAPH>>().second ) , typename U = decldecay_t( declval<GRAPH>().Edge( declval<inner_t<GRAPH>>() ).front().second )>
...
public:
  ...
  // O(min(\sum_{t1} |V_{t1}|^2,\sum_{t1} (|V_{t1}| + |E_{t1}| log_2|V_{t2}|)))
  // で計算する。
