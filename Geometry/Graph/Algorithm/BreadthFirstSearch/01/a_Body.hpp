...
template <typename T , typename GRAPH> inline void ZeroOneBreadthFirstSearch<T,GRAPH>::Initialise( const T& init ) { auto&& i = m_G.Enumeration_inv( init ); assert( 0 <= i && i < size() ); Initialise(); m_next <<= init; m_found[i] = 2; m_weight[i] = 0; }
template <typename T , typename GRAPH> inline void ZeroOneBreadthFirstSearch<T,GRAPH>::Initialise( vector<T> inits ) { Initialise(); for( auto& init : inits ){ m_next <<= move( init ); } const int& V = size(); for( auto u : m_next ){ auto&& i = m_G.Enumeration_inv( u ); assert( 0 <= i && i < V ); m_found[i] = 2; m_weight[i] = 0; } }
