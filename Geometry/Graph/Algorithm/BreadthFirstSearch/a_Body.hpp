...
template <typename T , typename GRAPH> template <template <typename...> typename VEC> inline void VirtualBreadthFirstSearch<T,GRAPH>::Initialise( VEC<T> inits ) { Initialise(); for( auto& init : inits ){ m_next.push_back( move( init ) ); } const int& V = size(); for( auto& u : m_next ){ auto&& i = m_G.Enumeration_inv( u ); assert( 0 <= i && i < V ); m_found[i] = true; } }
...
template <typename T , typename GRAPH> template <template <typename...> typename VEC> inline void VirtualBreadthFirstSearch<T,GRAPH>::Shift( VEC<T> inits ) { if( m_initialised ){ m_next.clear(); const int& V = size(); for( auto& u : m_next ){ auto&& i = m_G.Enumeration_inv( u ); assert( 0 <= i && i < V ); if( ! m_found[i] ){ m_next.push_back( u ); m_found[i] = true; } } } else { Initialise( move( inits ) ); } }
