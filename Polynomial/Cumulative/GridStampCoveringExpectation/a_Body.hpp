...
template <typename T>
T GridBigStampCoveringExpectation( const int& H ,  const int& W , const int& X ,  const int& Y , const int& N )
{
  ...
  for( int i = 1 ; i <= N ; i++ ){
    ...
    auto factor = [&]( const int& h , const int& k ){
...
template <typename T>
T GridSmallStampCoveringEpxpectation( const int& H ,  const int& W ,  const int& X ,  const int& Y , const ll& N )
{
...
template <typename T> inline T GridStampCoveringEpxpectation( const int& H ,  const int& W ,  const int& X ,  const int& Y , const ll& N ) { assert( 1 <= X && X <= H && 1 <= Y && Y <= W ); return ll( N ) * N <= ll( X ) * Y ? GridBigStampCoveringEpxpectation<T>( int( H ) , int( W ) , int( X ) , int( Y ) , int( N ) ) : GridSmallStampCoveringEpxpectation<T>( H , W , X , Y , N ); }
