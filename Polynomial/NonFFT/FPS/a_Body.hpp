// c:/Users/user/Documents/Programming/Mathematics/Polynomial/NonFFT/FPS/a_Body.hpp
...
FormalPowerSeries<T>& FormalPowerSeries<T>::operator*=( Polynomial<T> f )
{
  ...
  for( int i = this->m_N - 1 ; i >= 0 ; i-- ){
    ...
    for( int j = min( size_f , this->m_N - i ) - 1 ; j > 0 ; j-- ){
...
template <typename T> inline FormalPowerSeries<T>& FormalPowerSeries<T>::operator/=( const FormalPowerSeries<T>& f ) { assert( m_N <= f.m_N ); return operator*=( m_N == f.m_N ? f.Inverse() : FormalPowerSeries<T>( m_N , f ).Inverse() ); }
...
FormalPowerSeries<T> ShiftedIntegral( const int& n , const FormalPowerSeries<T>& f , const int& shift ) {
  ...
  if( n == 0 ){
    ...
    const int i_min = max( shift - 1 , 0 );
    T denom = i_min;

    for( int i = i_min ; i < size ; i++ ){
    ...
    const int i_min = max( shift - n , 0 );

    for( int i = i_min ; i < size ; i++ ){

      F[i + n - shift] = f[i] * T::Factorial( i ) * T::FactorialInverse( n + i );
...
FormalPowerSeries<T> Exp( const FormalPowerSeries<T>& f )
{
  ...
  while( power_2 < N ){
    ...
    temp /= f_exp;
    temp = ShiftedIntegral( 1 , temp , power );
