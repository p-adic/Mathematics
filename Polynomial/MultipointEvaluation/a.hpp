...
template <typename T , template <typename...> typename V1 , template <typename...> typename V2> void SetQuotientPartialFractionDecomposition( const Polynomial<T>& f , const V1<T>& point , V2<T>& answer );
// pointに重複がない場合にf(x) ≡ h(x)/Π(x-p) mod x^{M+1}かつdeg(h(x)) <= Mを満たす唯一の
// 多項式h(x)に対するh(x)/Π(x-p)の部分分数分解の定数項と1/(x-p)の係数をanswerに格納する。
// （O(D + M(log M)^2)）
template <typename T , template <typename...> typename V1 , template <typename...> typename V2> void SetRationalPartialFractionDecomposition( Polynomial<T> f , const V1<T>& point , V2<T>& answer );
