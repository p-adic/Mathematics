...
// AがF_2係数L×M行列（係数行列）とF_2係数L次縦ベクトル1個（非斉次項行列）を結合した
// 拡大係数行列である場合に
// Aに対し係数行列部分の簡約化＋拡大係数行列の左端非零成分が1である階段化を施し、
// {係数行列の階数,解が存在する?解:0次正方行列}を返す（O(L bound_M_N min{L,M})）。
// 解が存在する場合の一意性は係数行列の階数がMか否かで判定可能。
template <size_t bound_M_N> pair<size_t,vector<bool>> ExtendedReducedRowEchelonForm( vector<bitset<bound_M_N>>& A , const size_t& M );
...
// AがF_2係数L×M行列である場合にのみサポート。
// Aのrankを返す。（O(bound_M L min{L,M})）
template <size_t bound_M> inline size_t Rank( vector<bitset<bound_M>> A , const size_t& M );
...
template <size_t bound_L , size_t bound_M> pair<int,vector<bool>> LinearRelation( const vector<bitset<bound_M>>& A , const size_t& M );
