...
template <typename U , typename N_MODULE> inline GeometricProgressionMinAction<U,N_MODULE>::GeometricProgressionMinAction( N_MODULE M , const vector<U>& memory_r ) : m_M( move( M ) ) , m_p_memory_r( &memory_r ) { static_assert( is_same_v<U,inner_t<N_MODULE>> ); }
...
pair<U,int> GeometricProgressionMinAction<U,N_MODULE>::Action( const vector<U>& y , pair<U,int> u )
{
  ...
  for( int i = 0 ; i < size ; i++ ){
    ...
    u.first = min( u.first , m_M.Product( m_M.Power( ( *m_p_memory_r )[i] , u.second ) , y[i] ) );
...
template <typename U , typename N_MODULE> inline void GeometricProgressionMinDualSqrtDecomposition<U,N_MODULE>::IntervalSetMin( const int& i_start , const int& i_final , const int& r_num , const U& y ) { assert( 0 <= r_num && r_num < int( m_memory_r.size() ) ); BaseOfGeometricProgressionMinDualSqrtDecomposition<U,N_MODULE>::IntervalAct( i_start , i_final , pair<int,U>{ r_num , y } ); }
