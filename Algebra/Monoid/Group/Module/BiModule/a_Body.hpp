...
template <typename L , typename R , typename O_U_L , typename O_U_R , typename GROUP , typename U> inline AbstractBiModule<L,R,O_U_L,O_U_R,GROUP,U>::AbstractBiModule( const L& dummy_l , const R& dummy_r , O_U_L o_U_L , O_U_R o_U_R , GROUP M ) : GROUP( move( M ) ) , m_o_U_L( move( o_U_L ) ) , m_o_U_R( move( o_U_R ) ) { static_assert( is_same_v<U,inner_t<GROUP>> && is_invocable_r_v<U,O_U_L,const L&,U> && is_invocable_r_v<U,O_U_R,U,const R&> ); }

template <typename L , typename R , typename O_U_L , typename O_U_R , typename GROUP , typename U> inline U AbstractBiModule<L,R,O_U_L,O_U_R,GROUP,U>::LAction( const L& l , U u ) { return m_o_U_L( l , move( u ) ); }
template <typename L , typename R , typename U> inline U BiModule<L,R,U>::LAction( const L& l , U u ) { return move( u *= l ); }

template <typename L , typename R , typename O_U_L , typename O_U_R , typename GROUP , typename U> inline U AbstractBiModule<L,R,O_U_L,O_U_R,GROUP,U>::RAction( U u , const R& r ) { return m_o_U_R( move( u ) , r ); }
