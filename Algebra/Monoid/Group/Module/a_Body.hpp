...
template <typename MAGMA , typename U> inline RegularRSet<MAGMA,U>::RegularRSet( MAGMA magma ) : MAGMA( move( magma ) ) {}
...
template <typename R , typename O_U , typename GROUP , typename U> inline AbstractModule<R,O_U,GROUP,U>::AbstractModule( const R& dummy , O_U o_U , GROUP M ) : AbstractRSet<R,U,O_U>( dummy , M.One() , move( o_U ) ) , GROUP( move( M ) ) { static_assert( is_same_v<U,inner_t<GROUP>> ); }
...
template <typename MAGMA , typename U> inline U RegularRSet<MAGMA,U>::Action( const U& r , U u ) { return this->Product( r , move( u ) ); }
